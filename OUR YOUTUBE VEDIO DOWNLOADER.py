...
windo = Tk()
windo.configure(background='light grey')
windo.title("YouTube Video Downloader")

windo.geometry('1220x627') 
...
def get_info():
    ...
    im5_resized = im5.resize((250, 350), PIL.Image.ANTIALIAS)
...
def download_video_thumbnail():
    try:
        ...
        im5.save(vid_id+'.jpg')
        msg = tk.Label(windo, text='Thumbnail Downloaded', width=25, height=2, fg="white", bg="black",
                        font=('times', 18, ' bold '))
...
pred = tk.Label(windo, text=" MY YouTube Downloader", width=30, height=2, fg="white", bg="maroon2",
                font=('times', 25, ' bold '))
